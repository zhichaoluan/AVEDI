...
import numpy as np
from SimPEG.utils.model_builder import getIndicesBlock
...
def rand_rect(x_bound, y_bound, w_range, h_range, mesh, num_rect):
    ...
    for _ in range(num_rect):
        ...
        # return tuple. 1st is 1D integer array
...
def rand_circle(center_x_bound, center_y_bound, radius_bound, mesh, num_circle):
    ...
    for _ in range(num_circle):
        ...
        circle_idx = getIndicesSphere(radius, center, mesh .gridCC)  # return 1D bool array
...
def get_random_model(config_file, mesh, num_examples=None):
    ...
    for _ in range(num_examples):
        ...
        resistivity = smooth2d(model.reshape(mesh.nCy, mesh.nCx),
                               kernel_shape)
        # shape is (nz, nx)
